...
function MulticaControlCenter({ navigate }) {
  ...
  function patchAgent(values) {
    setAgentForm(current => ({ ...current, ...values }))
  }

  function patchProject(values) {
    setProjectForm(current => ({ ...current, ...values }))
  }

  function patchRepo(values) {
    setRepoForm(current => ({ ...current, ...values }))
  }

  function patchIssue(values) {
    setIssueForm(current => ({ ...current, ...values }))
  }

  function patchAssign(values) {
    setAssignForm(current => ({ ...current, ...values }))
  }
      ...
      <section className="agent-workspace">
        <header className="topbar">
          <div>
            ...
            <h2>Manage agents and route work from this training app.</h2>
          </div>
          <div className={`health-pill ${daemonReady ? '' : 'warning'}`}>
            <Server size={16} aria-hidden="true" />
            {daemonReady ? 'Daemon reachable' : 'Needs attention'}
          </div>
        </header>
        ...
        <div className="agent-grid">
          ...
          <section className="panel agent-panel">
            ...
            <div className="agent-form-stack">
              <label>
                Agent name
                <input value={agentForm.name} onChange={event => patchAgent({ name: event.target.value })} placeholder="Frontend reviewer" />
              </label>
              <label>
                Runtime
                <select value={agentForm.runtimeId} onChange={event => patchAgent({ runtimeId: event.target.value })}>
              ...
              <label>
                Description
                <input value={agentForm.description} onChange={event => patchAgent({ description: event.target.value })} placeholder="What this agent is for" />
              </label>
              <label>
                Instructions
                <textarea value={agentForm.instructions} onChange={event => patchAgent({ instructions: event.target.value })} placeholder="Focus on tests, security, and small commits." />
              </label>
              <label>
                Visibility
                <select value={agentForm.visibility} onChange={event => patchAgent({ visibility: event.target.value })}>
          ...
          <section className="panel agent-panel">
            ...
            <div className="agent-form-stack">
              <label>
                Project title
                <input value={projectForm.title} onChange={event => patchProject({ title: event.target.value })} placeholder="Company app" />
              </label>
              <label>
                Description
                <input value={projectForm.description} onChange={event => patchProject({ description: event.target.value })} placeholder="Optional project context" />
              </label>
              <label>
                Attach repo URL
                <input value={projectForm.repoUrl} onChange={event => patchProject({ repoUrl: event.target.value })} placeholder="https://github.com/company/app" />
              </label>
              <button
                type="button"
                className="primary"
              ...
              <label>
                Workspace repo URL
                <input value={repoForm.url} onChange={event => patchRepo({ url: event.target.value })} placeholder="git@github.com:company/app.git" />
              </label>
              <button
                type="button"
                disabled={Boolean(busyAction)}
          ...
          <section className="panel agent-panel">
            ...
            <div className="agent-form-stack">
              <label>
                Title
                <input value={issueForm.title} onChange={event => patchIssue({ title: event.target.value })} placeholder="Fix login validation" />
              </label>
              <label>
                Description
                <textarea value={issueForm.description} onChange={event => patchIssue({ description: event.target.value })} placeholder="Describe exact expected behavior and files to inspect." />
              </label>
              <div className="agent-form-pair">
                <label>
                  Status
                  <select value={issueForm.status} onChange={event => patchIssue({ status: event.target.value })}>
                ...
                <label>
                  Priority
                  <select value={issueForm.priority} onChange={event => patchIssue({ priority: event.target.value })}>
              ...
              <label>
                Project
                <select value={issueForm.projectId} onChange={event => patchIssue({ projectId: event.target.value })}>
              ...
              <label>
                Assign to agent
                <select value={issueForm.assigneeId} onChange={event => patchIssue({ assigneeId: event.target.value })}>
          ...
          <section className="panel agent-panel">
            ...
            <div className="agent-form-stack">
              <label>
                Issue
                <select value={assignForm.issueId} onChange={event => patchAssign({ issueId: event.target.value })}>
              ...
              <label>
                Agent
                <select value={assignForm.assigneeId} onChange={event => patchAssign({ assigneeId: event.target.value })}>
    ...
    <main className="landing">
      <nav className="landing-nav">
        ...
        <div className="nav-actions">
          ...
          <button type="button" onClick={() => navigate('/agents')}>
            <Bot size={16} aria-hidden="true" /> Agents
          </button>
      ...
      <section className="entry-grid">
        ...
        <article className="entry-card">
          ...
          <h3>Multica Control Center</h3>
          <p>Manage real local coding agents, runtimes, projects, issues, and command logs through a safe CLI bridge.</p>
          <button type="button" onClick={() => navigate('/agents')}>Open agents</button>
        </article>
        <article className="entry-card">
